...
module.exports = {
    getAll: async (query=null) => {
        ...
        if (capacity === undefined && plantType === undefined){
            return await PowerPlant.find({}).select('-geo').lean()
        }
        if (capacity === 'below 50MW'){
            return await PowerPlant.find({'properties.capacityInMW':{$lt: 50}}).select('-geo').lean() 
        }
        
        if(capacity === 'above 50MW'){
            return await PowerPlant.find({'properties.capacityInMW':{$gt: 50}}).select('-geo').lean() 
        }
        
        if(plantType !== null){
            return await PowerPlant.find({'properties.plantType': plantType}).select('-geo').lean()
        } 
    },
    ...
    addNew: async(body) => await PowerPlant.create(body)
}
